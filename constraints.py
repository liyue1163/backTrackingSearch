...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        for valuei in qi.curDomain():
            for valuej in qj.curDomain():
                diag = abs(valuei - valuej) == abs(i - j)
                if not diag and valuei != valuej:
                     satisfyingAssignments.append([valuei, valuej])
...
class NValuesConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        scope = self.scope()[:]
        scope.remove(var)
        for cur_var in scope:
            if cur_var.isAssigned():
                if cur_var.getValue() == self._required:
                    cnt_req += 1
            else:
                if self._required in cur_var.curDomain() and cur_var.curDomainSize() == 1:
                    # not assigned, but must assigned to required
                    cnt_req += 1
                if self._required in cur_var.curDomain() and cur_var.curDomainSize() > 1:
                    # could be assigned to required
                    cnt_req_potential += 1
        if cnt_req > self._ub:
            return False
        if self._lb <= cnt_req <= self._ub:
            return True
        if cnt_req < self._lb:
            if cnt_req + cnt_req_potential >= self._lb:
                return True
            return False
        print("should NOT !!")
