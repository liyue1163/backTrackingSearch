...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            row_cons = AllDiffConstraint("row_cons", row)
            constraint_list.append(row_cons)
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            col_con = AllDiffConstraint("col_cons", scope)
            constraint_list.append(col_con)
            # util.raiseNotDefined()
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                sub_cons = AllDiffConstraint("sub_cons", scope)
                constraint_list.append(sub_cons)
                # util.raiseNotDefined()
